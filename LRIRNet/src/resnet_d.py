...
class ResNet_(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 56, 40, layers[0])
        self.layer2 = self._make_layer(block, 56, 40, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 64 + 24, 40, layers[2], stride=2)
        self.layer4 = self._make_layer(block, 144, 40, layers[3], stride=2)
        self.avgpool = nn.AvgPool2d(7, stride=1)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
...
def resnet18(pretrained=True, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load("./resnet18-deep-inplane128.pth", map_location="cpu"))
    return model


def resnet18_(pretrained=False, **kwargs):
    """Constructs a ResNet-18_ model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_(BasicBlock, [1, 1, 1, 1], **kwargs)
    if pretrained:
        model.load_state_dict(torch.load("./resnet18-deep-inplane128.pth", map_location="cpu"))
    return model
